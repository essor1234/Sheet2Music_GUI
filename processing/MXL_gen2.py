...
def parse_clef_info(filename: str) -> tuple[str, int]:
    ...
    try:
        match = re.search(r'clef_(\d+)_([gf])Clef', filename, re.IGNORECASE)
        if match:
            staff_num = int(match.group(1))
            clef_type = match.group(2).upper()  # Convert 'g' or 'f' to 'G' or 'F'
            return clef_type, staff_num
        else:
            print(f"Warning: No clef information found in {filename}, defaulting to G-clef for staff 1")
            return 'G', 1
    except Exception as e:
...
def validate_note_data(note_data: Dict[str, Any]) -> bool:
    ...
    return all(key in note_data for key in required) and isinstance(note_data['bbox'], list) and len(note_data['bbox']) >= 1
    ...
    for group_id, group_data in pitch_results.items():
        ...
        for filename, notes in group_data.items():
            ...
            if measure_num not in organized_data[group_id]:
                organized_data[group_id][measure_num] = {'staff_1': [], 'staff_2': []}
            ...
            if staff_num == 1:
                organized_data[group_id][measure_num]['staff_1'].extend(chords)
            elif staff_num == 2:
                organized_data[group_id][measure_num]['staff_2'].extend(chords)
...
'''
    ...
    measure_number = 1
    group_ids = sorted(organized_data.keys(), key=lambda x: (int(re.search(r'page_(\d+)', x).group(1)), int(re.search(r'group_(\d+)', x).group(1))))
    ...
    for group_index, group_id in enumerate(group_ids):
        ...
        for filename in pitch_results.get(group_id, {}):
            clef_type, staff_num = parse_clef_info(filename)
            if staff_num == 1 and clef_staff_1 is None:
                clef_staff_1 = clef_type
            elif staff_num == 2 and clef_staff_2 is None:
                clef_staff_2 = clef_type
        if clef_staff_1 is None:
            clef_staff_1 = 'G'
        if clef_staff_2 is None:
            clef_staff_2 = 'F'
...
'''
        ...
        for measure_num in range(max_measure + 1):
            notes = organized_data[group_id].get(measure_num, {'staff_1': [], 'staff_2': []})
            staff_1_chords = sorted(notes['staff_1'], key=lambda x: x[0]['bbox_x'] if x else float('inf'))
            staff_2_chords = sorted(notes['staff_2'], key=lambda x: x[0]['bbox_x'] if x else float('inf'))

            if measure_num > 0:
                musicxml_content += f'''    <measure number="{measure_number}">
'''

            for chord in staff_1_chords:
...
'''
                        ...
                        print(f"Warning: Skipping invalid note in {group_id}, measure {measure_num}, staff_1: {note_data}, error: {e}")

            for chord in staff_2_chords:
                for i, note_data in enumerate(chord):
                    try:
                        musicxml_content += '''      <note>
'''
...
'''
                        ...
                        print(f"Warning: Skipping invalid note in {group_id}, measure {measure_num}, staff_2: {note_data}, error: {e}")

            if not staff_1_chords and not staff_2_chords:
                musicxml_content += '''      <note>
        <rest/>
        <duration>40320</duration>
        <voice>1</voice>
...
def display_or_play_score(musicxml_content: str, mode: str = 'auto', filename: str = 'temp_musicxml.musicxml') -> bool:
    ...
    try:
        ...
        has_notation_software = notation_software is not None and os.path.exists(notation_software)
...
def process_musicXML_generating1(
    pitch_results,
    pdf_path=None,
    output_dir="results",
    is_display=True,
