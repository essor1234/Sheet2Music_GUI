...
class StaffLineDetector:
    def __init__(self, grouping_path, pdf_path, is_display=False, extra=None):
        self.grouping_path = Path(grouping_path)
    ...
    @staticmethod
    def get_staff_line_y_coordinates(image_path, is_display=True):
        img = cv2.imread(str(image_path), cv2.IMREAD_GRAYSCALE)
        _, binary = cv2.threshold(img, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
        row_mean = np.mean(binary, axis=1)
        threshold = np.max(row_mean) * 0.5
        staff_row_indices = np.where(row_mean > threshold)[0]
    ...
    @staticmethod
    def calculate_staff_line_distance(first, last):
        return (last - first) / 4

    @staticmethod
    def calculate_staff_y_coordinates(first_line_y, distance, staff_type="line", extra=None):
        if staff_type == "line":
            start, end = 1, 10
        elif staff_type == "space":
            start, end = 0, 12
        else:
        ...
        staff_list = []
        for n in range(start, end, 2):
            y = int(first_line_y - distance / 2 + n * distance / 2)
            staff_list.append((n, y))

        return staff_list

    def process(self):
        pdf_dir = self.grouping_path / self.pdf_name
        ...
        for page_dir in pdf_dir.glob("page_*"):
            for group_dir in page_dir.glob("group_*"):
                ...
                group_key = f"{self.pdf_name}/{page_dir.name}/{group_dir.name}"
                ...
                for img_path in image_files:
                    print(f"Processing: {img_path.name}")
                    staff_lines = self.get_staff_line_y_coordinates(img_path, is_display=self.is_display)

                    if not staff_lines or len(staff_lines) < 2:
                        print(f"⚠️ Insufficient staff lines in {img_path.name}, skipping")
                        continue

                    distance = self.calculate_staff_line_distance(staff_lines[0], staff_lines[-1])
                    line_coords = self.calculate_staff_y_coordinates(staff_lines[0], distance, "line", self.extra)
                    space_coords = self.calculate_staff_y_coordinates(staff_lines[0], distance, "space", self.extra)

                    self.results[group_key][img_path.name] = {
                        "staff_lines": line_coords,
                        "staff_spaces": space_coords
                    }
