...
class StaffLineSeparator:
    def __init__(self, filter_val=160, is_display=False):
        self.filter_val = filter_val
        self.is_display = is_display

    def separate_staff_line(self, img):
        if img is None:
            raise ValueError("Input image is None")
        ...
        staff_lines = np.full_like(binary_otsu, 255, dtype=np.uint8)
        row_means = np.mean(binary_otsu, axis=1)
        staff_rows = np.where(row_means < self.filter_val)[0]

        if staff_rows.size > 0 and np.max(staff_rows) >= binary_otsu.shape[0]:
            staff_rows = staff_rows[staff_rows < binary_otsu.shape[0]]
        ...
        img_no_staff = cv2.inpaint(binary_otsu, mask, inpaintRadius=3, flags=cv2.INPAINT_TELEA)
        staff_lines[mask == 255] = 0

        if self.is_display:
            self._display_images(binary_otsu, img_no_staff, staff_lines)

        return img_no_staff, staff_lines

    def _display_images(self, binary, removed, lines):
        for title, image in zip(["Original", "No Staff", "Staff Lines"], [binary, removed, lines]):
            print(title)
            plt.imshow(image, cmap='gray')
            plt.axis('off')
            plt.show()

    def process_groups_dataset(self, groups_path, output_no_staff_dir, output_staff_dir):
        os.makedirs(output_no_staff_dir, exist_ok=True)
        os.makedirs(output_staff_dir, exist_ok=True)

        group_folders = [f for f in os.listdir(groups_path) if f.startswith('group_') and os.path.isdir(os.path.join(groups_path, f))]
        if not group_folders:
            print(f"⚠️ No group folders found in {groups_path}")
            return

        for group in group_folders:
            in_dir = os.path.join(groups_path, group)
            out_no_staff = os.path.join(output_no_staff_dir, group)
            out_staff = os.path.join(output_staff_dir, group)
            os.makedirs(out_no_staff, exist_ok=True)
            ...
            for fname in image_files:
                ...
                if img is None:
                    ...
                    continue
                try:
                    no_staff, staff = self.separate_staff_line(img)
                    cv2.imwrite(os.path.join(out_no_staff, fname), no_staff)
                    cv2.imwrite(os.path.join(out_staff, fname), staff)
    ...
    def separate_staff_from_clefs_flat(self, clef_input_root, output_no_staff_root, output_staff_root):
        pdf_name = os.path.basename(clef_input_root.rstrip("/\\"))
        no_staff_root = os.path.join(output_no_staff_root, pdf_name)
        staff_root = os.path.join(output_staff_root, pdf_name)
        os.makedirs(no_staff_root, exist_ok=True)
        os.makedirs(staff_root, exist_ok=True)

        page_dirs = [d for d in os.listdir(clef_input_root) if d.startswith("page_") and os.path.isdir(os.path.join(clef_input_root, d))]
        for page in page_dirs:
            page_input = os.path.join(clef_input_root, page, "clef")
            if not os.path.isdir(page_input):
                print(f"⚠️ Skipping {page}: No 'clef' folder")
                continue

            out_no_staff = os.path.join(no_staff_root, page, "clef")
            out_staff = os.path.join(staff_root, page, "clef")
            os.makedirs(out_no_staff, exist_ok=True)
            os.makedirs(out_staff, exist_ok=True)

            image_files = [f for f in os.listdir(page_input) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
            for fname in image_files:
                img_path = os.path.join(page_input, fname)
                img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
                if img is None:
                    print(f"❌ Failed to load {fname}")
                    continue
                try:
                    no_staff, staff_only = self.separate_staff_line(img)
                    cv2.imwrite(os.path.join(out_no_staff, fname), no_staff)
                    cv2.imwrite(os.path.join(out_staff, fname), staff_only)
                    print(f"✅ Saved: {fname}")
                except Exception as e:
                    print(f"⚠️ Error on {fname}: {e}")

        print("\n✅ Staff line removal complete.")
        return no_staff_root, staff_root